package com.example.coalesce.coordinator;

import com.example.coalesce.core.CoalesceEnvelope;
import com.example.coalesce.core.CoalesceState;
...
public class RedissonCoalesceCoordinator {
    ...
    private RBucketReactive<byte[]> bucket(String key) {
        return redisson.getBucket(discriminate(key, "state"), ByteArrayCodec.INSTANCE);
    }
    ...
    private RTopicReactive topic(String key) {
        ...
        return redisson.getTopic(discriminate(key, "notify"), StringCodec.INSTANCE);
    }

    private String lockKey(String key) {
        return discriminate(key, "lock");
    }

    /** {@code coalesce:{raw}} + {@code lock} becomes {@code coalesce:lock:{raw}}. */
